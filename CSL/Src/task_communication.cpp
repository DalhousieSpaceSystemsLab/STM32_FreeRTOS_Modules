...
typedef struct _QueueHandleNode {
...
bool csl::send_message(csl::SubSystem subsystem, char* ptr, unsigned int len) {
    if (len > CSL_QUEUE_ITEM_SIZE) return false;
    
    QueueHandle_t handle = _get_queue(subsystem);
...
bool csl::receive_message(csl::SubSystem subsystem, char ptr[sizeof(csl::_QueueMessage)], unsigned int *len) {
    QueueHandle_t handle = _get_queue(subsystem);
    if (subsystem == NULL) return false;
...
bool csl::_create_queue(csl:SubSystem subsystem) {
    if (queue_nodes_size == CSL_TOTAL_QUEUES) return false;
    
    QueueHandleNode node;
    
    node.handle = xQueueCreate(CSL_QUEUE_MAX_ITEMS, sizeof(csl::_QueueMessage));
...
QueueHandle_t csl::_get_queue(csl::SubSystem subsystem) {
